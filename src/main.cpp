...
int main() {
  pt::ptree config;
  pt::ini_parser::read_ini("conf/config.conf", config);

  // Get the threshold value from the configuration
  deviceNumber = config.get<int>("device", deviceNumber);
  virtualControllers =
      config.get<int>("virtual_controllers", NUM_VIRTUAL_CONTROLLERS);
  continuousPollingDelay =
      config.get<int>("continuous_polling_delay", continuousPollingDelay);
  // double threshold = config.get<double>("threshold", 0.8);
  if (deviceNumber < 0) {
