...
#include "../include/virtualController.hpp"
#include <fcntl.h>
#include <iostream>
...
VirtualController::VirtualController(bool printDebugInfoToConsole)
    : fd_virtual_device(-1), buttonStates(KEY_MAX, false),
      debugging(printDebugInfoToConsole),
...
void VirtualController::setButtonState(int buttonIndex, bool state = false) {
...
void VirtualController::toggleButtonState(int buttonIndex) {
  printf("buttostate %x\n",getButtonState(buttonIndex));
  setButtonState(buttonIndex, !getButtonState(buttonIndex));
  printf("buttostate %d\n",getButtonState(buttonIndex));

}
bool VirtualController::getButtonState(int buttonIndex) {
...
void VirtualController::setAbsoluteValue(int absIndex, float value = 0.0f) {
...
float VirtualController::getAbsoluteValue(int absIndex) {
  if (absIndex > ABS_VALUES_ON_CONTROLLER || absIndex < 0) // safety check
    return 0.0f;
...
void VirtualController::setControllerState(State newState) {
  controllerState = newState;
  updateControllerState();
}
void VirtualController::setControllerState() { clearControllerState(); }
void VirtualController::clearControllerState() {
  controllerState = State(BUTTONS_ON_CONTROLLER, ABS_VALUES_ON_CONTROLLER);
  updateControllerState();
}
State VirtualController::getControllerState() { return controllerState; }
...
void VirtualController::sync(){

  printf("AYe AYe WHAT ARE YOU SYNCING ABOUT");
...
void VirtualController::debugPrintState(
    bool override = false) // NOT WORKING AS INTENDED, also not important.
{
  if (!override && !debugging)
    return;
